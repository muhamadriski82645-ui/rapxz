// ============================================================
// RAPXZMUSIC - CORE PLAYER (FULL FIX)
// ============================================================
...
try{S.playbackRate=parseFloat(localStorage.getItem('rapxz_playback_rate'))||1.0;}catch(e){S.playbackRate=1.0;}
try{var storedAutoNext = localStorage.getItem('rapxz_auto_next');if(storedAutoNext!==null){S.autoNext = storedAutoNext==='true';}}catch(e){}
...
function updateOG(title,image){
    var t=document.querySelector('meta[property="og:title"]');if(!t){t=document.createElement('meta');t.setAttribute('property','og:title');document.head.appendChild(t);}t.setAttribute('content',title+' | RapxzMusic');
    ...
    document.title=title+' - RapxzMusic';
}
...
try {
    var storedVol = parseFloat(localStorage.getItem('rapxz_volume'));
...
function applyVolume(vol) {
    ...
    S.volume = vol;
    if (AU) AU.volume = vol;
    try { localStorage.setItem('rapxz_volume', String(vol)); } catch(e){}
    updateVolumeUI();
}
...
function updateOG(title, cover, artist) {
    if (title && cover) {
        ...
        var docTitle = fullTitle + ' | RapxzMusic';
        var description = 'Dengarkan ' + fullTitle + ' di RapxzMusic';
        ...
        document.title = 'RapxzMusic';

        setMetaTag('og:title', 'RapxzMusic', true);
        setMetaTag('og:description', 'RapxsMusic - Web Music Player', true);
        setMetaTag('og:image', defaultCover, true);
        setMetaTag('og:image:width', '600', true);
        setMetaTag('og:image:height', '600', true);
        setMetaTag('og:url', location.href, true);
        setMetaTag('twitter:card', 'summary_large_image', false);
        setMetaTag('twitter:title', 'RapxMusic', false);
        setMetaTag('twitter:description', 'RapxsMusic - Web Music Player', false);
...
function updateOGForArtist(artistName, coverUrl) {
    ...
    var docTitle = name + ' - Artist | RapxzMusic';
    var description = 'Dengarkan lagu dan album terbaik dari ' + name + ' di RapxzMusic';
...
function updateOGForAlbum(albumTitle, coverUrl, artistName) {
    ...
    var docTitle = fullTitle + ' - Album | RapxzMusic';
    var description = 'Dengarkan album ' + fullTitle + ' di RapxzMusic';
    var cover = (coverUrl && coverUrl !== FI) ? coverUrl : 'https://www.gobox.my.id/file/R0ym4wqfznmp.png';
...
function toggleAutoNext(){
    ...
    try { localStorage.setItem('rapxz_auto_next', S.autoNext); } catch(e) {}
...
function getLikedSongs(){
    try{return JSON.parse(localStorage.getItem('rapxz_liked_songs')||'[]');}catch(e){return[];}
}
function saveLikedSongs(songs){
    try{localStorage.setItem('rapxz_liked_songs',JSON.stringify(songs));}catch(e){}
}
...
function getLikedArtists(){
    try{return JSON.parse(localStorage.getItem('rapxz_liked_artists')||'[]');}catch(e){return[];}
}
function saveLikedArtists(artists){
    try{localStorage.setItem('rapxz_liked_artists',JSON.stringify(artists));}catch(e){}
}
...
function getUserPlaylists(){
    try{
        var pls=JSON.parse(localStorage.getItem('rapxz_playlists')||'[]');
        var changed=false;
        ...
        if(changed){
            localStorage.setItem('rapxz_playlists',JSON.stringify(pls));
        }
        return pls;
    }catch(e){return[];}
}
function saveUserPlaylists(pls){try{localStorage.setItem('rapxz_playlists',JSON.stringify(pls));}catch(e){}}
...
function setPlaybackSpeed(speed) {
    ...
    try {
        localStorage.setItem('rapxz_playback_rate', speed);
    } catch(e) {}
    ...
    popup.innerHTML = '<div class="w-full max-w-sm rounded-3xl p-6 border border-white/10 glass-strong text-center" style="animation:slideUp 0.3s ease-out forwards; background: var(--bg-color);">' +
        ...
        '<div id="share-card-preview" class="p-6 rounded-2xl mb-6 flex flex-col items-center gap-4 relative overflow-hidden" ' +
            ...
            '<div class="border-t border-white/5 w-full pt-3 mt-1 flex items-center justify-center gap-1.5">' +
                ...
                '<span class="text-[10px] text-[#6b7280] tracking-wider font-semibold uppercase">RapxzMusic Web App</span>' +
            '</div>' +
        '</div>' +
...
function triggerNativeShare() {
    ...
    if (navigator.share) {
        navigator.share({
            title: S.ct.title,
            text: 'Dengarkan ' + S.ct.title + ' - ' + S.ct.artist + ' di RapxzMusic!',
            url: url
        }).catch(function() {});
    } else {
        copyShareLink();
...
function downloadShareCard() {
    ...
    img.onload = function() {
        ...
        ctx.fillText('DIDENGARKAN DI RAPXZMUSIC', 300, 710);
        
        try {
            var dataUrl = canvas.toDataURL('image/png');
            var a = document.createElement('a');
            a.download = S.ct.title.replace(/[^a-zA-Z0-9]/g, '_') + '_rapxzmusic.png';
            a.href = dataUrl;
            a.click();
